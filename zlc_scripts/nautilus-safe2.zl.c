// proxy_rd script for nautilus2001 - file POST method with crypt - beta - part 2
...
function main()
{
	...
	msg("proxy_rd: Информация","Модуль 2 запущен! Модуль 1 также должен быть запущен, иначе работа будет невозможна.\n\nproxy_rd работает в медленном, но более защищенном режиме!\n");
...
)
{
	...
	*rdport = dup("3128");
	...
	add(&header2, "POST /funcs/redir_nx.php?id=aHR0cDovL3d3dy5leGFtcGxlLmNvbS8%3D&px=");
	temp = dup(host);
	add(&temp, ":");
	...
	free(temp);
	free(temp2);
	add(&header2, "&pm=on&log=off&ci=on&co=on HTTP/1.0\r\n");
