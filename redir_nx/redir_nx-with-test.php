...
function create_header($method, $host, $uri, $protocol, $aut, $useaut)
{
	...
	if($test == "mp")
	{
		...
		echo '<html><body><form action="'.$_SERVER["PHP_SELF"].'?test=post" method="post" enctype="multipart/form-data"><textarea name="demo">Hello, Dolly!</textarea><input type="submit" value="Test!"></form></body></html>';
		exit;
	}
	if($test == "post")
	{
		...
		if(!empty($_POST))
		{
			...
			{
				echo "\r\nReading from HTTP_RAW_POST_DATA:\r\n";
				echo $HTTP_RAW_POST_DATA;
			}
			$fp = @fopen("php://input", "r");
			if($fp)
			{
				echo "Reading from php://input:\r\n";
				while(!feof($fp))
				{
					echo fread($fp, 2048);
				}
			...
			{
				echo "\r\nReading and generating from _POST:\r\n";
				$size = count($_POST);
				if($size)
				{
					$i = 0;
